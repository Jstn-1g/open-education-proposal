...
2. Choose **Remix in the studio** from the play page, or **Make a copy** from the library. Edit the prompt, available pieces, hints and short challenge sequence. Changing a tray shows a gentle reminder to review its prompt and hint; the studio never silently rewrites them. Open “Learning goal & credits” to describe the purpose and contributor.
...
4. Try every challenge and undo. If the tray permits another construction, try it too. The validator checks structure and a possible route to one whole; it cannot check whether your writing teaches the right concept.
