...
`Interactive activity checks` runs on pull requests to `main`, pushes to `main`, manual dispatch, and as a reusable prerequisite for publication. It runs the recipe and exact-model tests, installs the declared Playwright and Chromium test tooling, builds a local test site, and checks Activity Studio and Learning Lab in Chromium. Its repository token is read-only, checkout credentials are not retained, and it does not deploy or upload artifacts.

Its concurrency group includes the caller's workflow name and branch, so a direct check run cannot cancel a Pages prerequisite on the same branch. New runs can still supersede older checks within the same workflow. This follows GitHub's [reusable-workflow context rules](https://docs.github.com/en/actions/reference/workflows-and-actions/reusing-workflow-configurations).

`Publish website manually` runs only when an authorized maintainer dispatches it from `main`. Its Pages build job waits for `Interactive activity checks` to pass. That prerequisite builds a test site before the publication guard runs. The Pages build job then calls `python -I -B release_check.py` and runs the standard-library tests before reading the Pages configuration, building the publication artifact, or uploading it. A failed prerequisite, missing or failing guard, or failed test prevents deployment. Incomplete launch facts must produce a nonzero result; do not remove the check to make a candidate deploy.
...
4. Dispatch `Publish website manually` from the reviewed `main` commit. Inspect the interactive checks, Pages build result, and deployment environment before approving any configured environment review.
